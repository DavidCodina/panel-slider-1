...
function indicate_slide(index){
  ...
  //
  //        slices[i].classList.remove("slide-out");
  //
  //
  ...
  for (let i = 0; i < slices.length; i++) {
    slices[i].classList.add("slide-out");
  }


  //For each slice having the same index value, within it's respective slice container, do this...
  for (let i = 0; i < slice_containers.length; i++){
    ...
    slice.classList.remove("slide-out");
